...
export class TimelineManager {
	...
	private recentTrackIds = new Set<string>();
	...
	addTrack({ type, index }: { type: TrackType; index?: number }): string {
		...
		this.recentTrackIds.add(trackId);
		this.editor.command.execute({ command });
		return trackId;
	}

	isRecentTrack(trackId: string): boolean {
		return this.recentTrackIds.has(trackId);
	}
	...
	insertElement({ element, placement }: InsertElementParams): {
		elementId: string;
		trackId: string | null;
	} {
	...
	applySilenceCutPlan({ targets }: { targets: SilenceCutTarget[] }): boolean {
		...
		const after = buildSilenceCutTracks({ tracks: before, targets });
		if (after === before) {
			return false;
		}

		const selectedElements = afterSelectionForSilenceTargets({
			tracks: after,
			targets,
		});
...
function afterSelectionForSilenceTargets({
	tracks,
	targets,
}: {
	tracks: SceneTracks;
	...
	const targetKeys = new Set(
		targets.map((target) => `${target.trackId}:${target.elementId}`),
	);
