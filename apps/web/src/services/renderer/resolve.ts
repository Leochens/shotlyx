...
import { buildRenderableTextElementFromSubtitle } from "@/subtitles/layer";
...
function resolveTextNode({
	...
	const background = buildTextBackgroundFromElement({ element: textElement });
	...
	return {
		...
		textColor: resolveColorAtTime({
			baseColor:
				typeof textElement.params.color === "string"
					? textElement.params.color
					: "#ffffff",
			animations: textElement.animations,
			propertyPath: "color",
			localTime,
