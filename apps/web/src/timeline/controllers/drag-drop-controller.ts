...
export class DragDropController {
	...
	private executeGraphicDrop({
		...
		if (dragData.motionGraphicAssetId && dragData.motionGraphicBaseParams) {
			const element = buildMotionGraphicElementFromAsset({
				asset: {
					id: dragData.motionGraphicAssetId,
					name: dragData.name,
					definitionId: dragData.definitionId,
					params: dragData.motionGraphicBaseParams,
					duration: dragData.duration ?? DEFAULT_NEW_ELEMENT_DURATION,
				},
				startTime: target.xPosition,
	...
	private executeMediaDrop({
		...
		if (!isTimelineMediaType(mediaAsset.type)) return;
		...
		await showMediaUploadToast({
			...
			promise: async () => {
				...
				for (const asset of processedAssets) {
					...
					if (!isTimelineMediaType(createdAsset.type)) continue;
