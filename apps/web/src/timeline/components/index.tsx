...
function TrackLabelsPanel({
	...
	return (
		...
		>
			<div className="shrink-0" style={{ height: timelineHeaderHeight || 48 }}>
						...
						<Tooltip>
							<TooltipTrigger asChild>
								<Button
									...
									variant="ghost"
									size="icon"
									aria-label="Select all timeline elements"
									className="size-8"
									disabled={allElementRefs.length === 0}
									onClick={(event) => {
										event.preventDefault();
										event.stopPropagation();
										setElementSelection({ elements: allElementRefs });
									}}
								>
			...
			<Tooltip>
				<TooltipTrigger asChild>
					<Button
						...
						size="icon"
						aria-label={label}
						className="size-7 opacity-60 transition hover:opacity-100 focus-visible:opacity-100"
						onClick={(event) => {
							event.preventDefault();
							event.stopPropagation();
							onClick();
