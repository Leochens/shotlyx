...
const createHugeiconsIcon = ({ icon }: { icon: IconSvgElement }) => {
	function AssetPanelTabIcon({ className }: { className?: string }) {
		return <HugeiconsIcon icon={icon} className={className} />;
	}
	return AssetPanelTabIcon;
};
...
interface AssetsPanelStore {
	...
	setSelectedAssetRefs: (refs: SelectedAssetRef[]) => void;
	clearSelectedAssetRefs: () => void;
...
export const useAssetsPanelStore = create<AssetsPanelStore>()(
	persist(
		(set) => ({
			...
			selectedAssetRefs: [],
			setSelectedAssetRefs: (refs) => set({ selectedAssetRefs: refs }),
			clearSelectedAssetRefs: () => set({ selectedAssetRefs: [] }),
			mediaViewMode: "grid",
