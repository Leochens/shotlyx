...
import {
	...
	buildShotlyxMGProgressAnimation,
...
export class ProjectManager {
	...
	async loadProject({ id }: { id: string }): Promise<void> {
		...
		try {
			...
			this.active = project;
			hydrateShotlyxMGAssets({
				assets: project.shotlyxMGAssets ?? [],
			});
			this.notify();
	...
	getFilteredAndSortedProjects({
		...
		const [rawKey, rawOrder] = sortOption.split("-");
		const key: TProjectSortKey =
			rawKey === "createdAt" ||
			rawKey === "updatedAt" ||
			rawKey === "name" ||
			rawKey === "duration"
	...
	getMotionGraphicAssets(): ProjectMotionGraphicAsset[] {
		return this.active?.motionGraphicAssets ?? [];
	}

	getShotlyxMGAssets(): ShotlyxMGAsset[] {
		return this.active?.shotlyxMGAssets ?? [];
	}

	getShotlyxMGAsset({ id }: { id: string }): ShotlyxMGAsset | null {
		return (
			(this.active?.shotlyxMGAssets ?? []).find((asset) => asset.id === id) ??
			null
		);
	}

	upsertShotlyxMGAsset({ asset }: { asset: ShotlyxMGAsset }): void {
		if (!this.active) return;
		...
		hydrateShotlyxMGAsset({ asset: nextAsset });
		if (hasExisting) {
			this.updateShotlyxMGInstancesForAsset({ asset: nextAsset });
		}
		this.editor.save.markDirty();
		this.notify();
	}

	removeShotlyxMGAsset({ id }: { id: string }): void {
		if (!this.active) return;
		const shotlyxMGAssets = (this.active.shotlyxMGAssets ?? []).filter(
			(asset) => asset.id !== id,
		);
		this.active = {
			...this.active,
	...
	getMotionGraphicAsset({
		id,
	}: {
		id: string;
	}): ProjectMotionGraphicAsset | null {
		return (
			(this.active?.motionGraphicAssets ?? []).find(
				(asset) => asset.id === id,
			) ?? null
		);
	}
	...
	upsertMotionGraphicAsset({
		...
		if (!this.active) return;
		const existing = this.active.motionGraphicAssets ?? [];
		const now = new Date().toISOString();
		const nextAsset: ProjectMotionGraphicAsset = {
			...asset,
			updatedAt: now,
			createdAt: asset.createdAt || now,
		};
		const hasExisting = existing.some((item) => item.id === asset.id);
		const motionGraphicAssets = hasExisting
			? existing.map((item) => (item.id === asset.id ? nextAsset : item))
			: [...existing, nextAsset];
		...
		if (hasExisting) {
			this.updateMotionGraphicInstancesForAsset({ asset: nextAsset });
		}
		this.editor.save.markDirty();
		this.notify();
	}

	removeMotionGraphicAsset({ id }: { id: string }): void {
		if (!this.active) return;
		const motionGraphicAssets = (this.active.motionGraphicAssets ?? []).filter(
			(asset) => asset.id !== id,
		);
		this.active = {
			...this.active,
	...
	private updateMotionGraphicInstancesForAsset({
		asset,
	}: {
		asset: ProjectMotionGraphicAsset;
	}): void {
		...
		const nextScenes = scenes.map((scene) => ({
			...
			tracks: {
				...scene.tracks,
				overlay: scene.tracks.overlay.map((track): OverlayTrack => {
					...
					const elements = track.elements.map((element) => {
						if (
							...
							element.motionGraphicAssetId !== asset.id
						) {
							return element;
						}
		...
		this.editor.scenes.setScenes({
			scenes: nextScenes,
			activeSceneId: this.editor.scenes.getActiveSceneOrNull()?.id,
		});
	}

	private updateShotlyxMGInstancesForAsset({
		asset,
	}: {
				...
				overlay: scene.tracks.overlay.map((track): OverlayTrack => {
					...
					const elements = track.elements.map((element) => {
						...
						return {
							...
							animations: buildShotlyxMGProgressAnimation({
								duration,
							}),
						};
					});
