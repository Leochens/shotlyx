import { completeDesktopExportTarget } from "@/desktop/export/server";
...
function errorStatus({ error }: { error: unknown }): number {
	...
	if (error.message === "desktop_export_empty_file") return 400;
	return 500;
}
...
export async function POST(request: Request) {
	...
	try {
		const result = await completeDesktopExportTarget({ targetId });
		return ApiResponse.json({
			desktop: true,
			...result,
		});
