...
function isContributor(value: unknown): value is Contributor {
	...
	return (
		...
		typeof get("login") === "string" &&
		typeof get("avatar_url") === "string" &&
		typeof get("html_url") === "string" &&
		typeof get("contributions") === "number" &&
...
async function getContributors(): Promise<Contributor[]> {
	try {
		const response = await fetch(
			"https://api.github.com/repos/shotlyx/shotlyx/contributors?per_page=100",
			{
				headers: {
		...
		const contributors = payload.filter(isContributor);
...
export default async function ContributorsPage() {
	...
	return (
		<BasePage
			title="Contributors"
			description="Meet the amazing people who contribute to Shotlyx, the AI-native video workspace."
		>
	...
	return (
		<div className="flex flex-col gap-12">
			<div className="flex flex-col gap-2 text-center">
				...
				<p className="text-muted-foreground">
					Everyone who makes Shotlyx better
				</p>
			</div>
	...
	return (
		<div className="flex flex-col gap-10">
			<div className="flex flex-col gap-2 text-center">
				...
				<p className="text-muted-foreground">Tools we use to build Shotlyx</p>
			</div>
