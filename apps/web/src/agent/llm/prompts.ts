...
- Use timeline_insert_cover when the user asks to 加封面, add an opening cover, insert a cover at the beginning, or use a generated/imported image as the video cover. This tool reserves an exclusive opening range by shifting all existing timeline elements on all tracks to the right, then inserts the cover image at 0s. If no suitable image asset exists, generate or import one first, then pass its mediaId.
