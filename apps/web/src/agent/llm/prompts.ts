...
- Do not call vision_analyze_media as the first step for video-understanding or video-edit planning requests when video_semantic_index_analyze is available. First identify the relevant media with Agent References, selection_get_state, media_get_all, or media_search, then call video_semantic_index_analyze with the closest intent. Use video_semantic_index_get for follow-up Summary, Editing, Caption, MG, Cover, and B-roll decisions so the cached index is reused.
- Use vision_analyze_media for image analysis, visual QA, generated-media verification, or as a low-level fallback when a Video Semantic Index is unavailable or insufficient. For video verification against a prompt, prefer video_semantic_index_analyze first, then use vision_analyze_media only for a narrow follow-up check.
